...
drone_err_t NRF_init(NRF_handle_t *radio, uint8_t CE_PIN, uint8_t CSN_PIN, uint8_t IRQ_PIN) {
  ...
  // I may change this back in future but I dont really need acks
...
drone_err_t NRF_set_address(NRF_handle_t *radio, NRF_addr_t rxAddr, NRF_addr_t txAddr) {
  ...
  memcpy(radio->rxAddr, rxAddr, sizeof(radio->rxAddr));
  memcpy(radio->txAddr, txAddr, sizeof(radio->txAddr));
  return DRONE_OK;
}
...
drone_err_t NRF_set_data_rate(NRF_handle_t *radio, data_rate_t dataRate) {
  ...
  default: // DATA_RATE_2MBPS
    txBuffer[1] = rxBuffer[1] & 0xDF; // [RF_DR_LOW, RF_DR_HIGH] = 01, RF_DR_HIGH is dont care
    txBuffer[1] = txBuffer[1] | 0x08;
    break;
  }
...
drone_err_t NRF_set_power_level(NRF_handle_t *radio, RF_power_level_t powerLevel) {
  ...
  {
  case MIN:
    txBuffer[1] = rxBuffer[1] & 0xF9; // 00
    break;
  
  case LOW:
    txBuffer[1] = rxBuffer[1] & 0xFB; // 01
    txBuffer[1] = rxBuffer[1] | 0x02;
    break;

  case HIGH:
    txBuffer[1] = rxBuffer[1] & 0xFD; // 10
    txBuffer[1] = rxBuffer[1] | 0x04;
    break;

  default: // MAX
    txBuffer[1] = rxBuffer[1] | 0x06; // 11
    break;
  }
...
drone_err_t NRF_enter_standby(NRF_handle_t *radio) {
  ...
  gpio_set_level(radio->CE_PIN, 0); // necessary
